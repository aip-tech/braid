...
`braid start` always runs both as a detached background daemon, not in the foreground — `pnpm dev` returns as soon as they're up, it doesn't block or print their output to this terminal. Persistent output lives in `.braid/logs/<name>.log`, tailed via `braid logs`.

```bash
pnpm dev             # start both (returns immediately once running), from the repo root or this directory
pnpm dev:status      # list PIDs/alive state
pnpm dev:logs        # tail both processes' output live (braid logs --follow)
