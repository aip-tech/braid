...
Once published, this will be `npx @aip-tech/braid start` or a `braid` script pointing at your own `braid.config.ts`. Today, from inside this workspace:

```bash
pnpm --filter ./packages/braid run build   # compiles src/ to dist/ (bin, main, and types all point there)
braid start             # run every configured process in the foreground
braid status            # list each process's name/pid/alive state from the pidfile
braid stop              # kill everything recorded in the pidfile, from a separate terminal
...
- The actual `npm publish` (and whatever `@aip-tech` org/access setup that needs).
- A web dashboard for start/stop/status, as a deliberate follow-up rather than part of this core.
